...
export function HeroSection({ onScrollToContact }: HeroSectionProps) {
  return (
    <section className="relative min-h-[90vh] pt-28 pb-8 md:pt-32 md:pb-12 overflow-hidden bg-[#faece3] flex flex-col justify-center">
      {/* Floating Objects */}
      <div className="absolute inset-0 pointer-events-none z-20 overflow-hidden">
        ...
        <motion.div
          ...
          className="hidden sm:block absolute top-[35%] left-[2%] md:left-[5%] w-32 h-32 md:w-48 md:h-48 bg-[#2b309b] rounded-lg shadow-2xl skew-x-12"
        />
        {/* Pink Table (Simulated) */}
        <motion.div
          animate={{ y: [0, 10, 0], rotate: [0, 5, 0] }}
          transition={{ duration: 5, repeat: Infinity, ease: "easeInOut", delay: 1 }}
          className="hidden sm:block absolute top-[10%] right-[10%] md:right-[20%] w-24 h-24 md:w-40 md:h-40 bg-[#ff9eb5] rounded-full shadow-xl flex items-center justify-center"
        >
        ...
        <motion.div
          ...
          className="hidden sm:block absolute top-[45%] left-[25%] md:left-[30%] w-24 h-32 md:w-40 md:h-48 bg-[#00873e] rounded-[3rem] shadow-2xl -rotate-12"
        />
        {/* Yellow Patterned Plate */}
        <motion.div
          animate={{ rotate: 360 }}
          transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
          className="hidden sm:block absolute bottom-[15%] left-[10%] md:left-[20%] w-32 h-32 md:w-56 md:h-56 bg-[#d4df00] rounded-full border-[8px] border-black border-dashed shadow-xl"
        />
        {/* Blue Vase */}
        <motion.div
          animate={{ y: [0, 15, 0] }}
          transition={{ duration: 6, repeat: Infinity, ease: "easeInOut", delay: 2 }}
          className="hidden sm:block absolute bottom-[5%] right-[25%] md:right-[35%] w-16 h-24 md:w-24 md:h-40 bg-[#8cb8d9] rounded-t-3xl rounded-b-xl shadow-xl"
        />
        {/* Colorful Striped Vase */}
        <motion.div
          animate={{ y: [0, -10, 0] }}
          transition={{ duration: 5, repeat: Infinity, ease: "easeInOut", delay: 1.5 }}
          className="hidden sm:block absolute top-[20%] left-[50%] w-12 h-20 md:w-20 md:h-32 bg-gradient-to-b from-yellow-400 via-pink-500 to-purple-500 rounded-full shadow-xl"
        />
      </div>
